...
function firebaseTrialSave(trialdata, successfulSaveCallback) {
  if (firebase) {
    ...
    } else if(trialdata.trialNumber == 'ballTrajMatrix') {
    // Save the Ball Trajectory data
    var ref = firebase.database().ref(expName + '/' + uid + '/' + subjID + '_' + inputdevice + '_' + startDate + '/BallTrajectories' + '/' + trialdata.conttrial + '/' + trialdata.ballcontTrajTrial);
    var datatoPost = trialdata.data;
  ...
  if (firebase) {
    ...
    ref.set(datatoPost, function(error) {
      if (error) {
        ...
        successfulSaveCallback();
        consoleLog("Incomplete trajectory cleared. (conttrial = " + (subjCrashedData.conttrial_crash +1) + ")")
      }
    });
...
function firebaseClearIncompleteBallTrajectory(successfulSaveCallback) {
  if (firebase) {
    
    var ref = firebase.database().ref(expName + '/' + subjCrashedData.uid_crash + '/' + subjCrashedData.subjID_crash + '_' + subjCrashedData.device_crash + '_' + subjCrashedData.startDate_crash + '/BallTrajectories' + '/' + (subjCrashedData.conttrial_crash +1));
    var datatoPost = [];
    ...
    ref.set(datatoPost, function(error) {
      if (error) {
        ...
        successfulSaveCallback();
        consoleLog("Incomplete ball trajectory cleared. (conttrial = " + (subjCrashedData.conttrial_crash +1) + ")")
      }
    });
  } else {
    consoleLog('ERROR: Not connected to Firebase.');
